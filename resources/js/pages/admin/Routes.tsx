...
// IMPORTANT: You must import the CSS for the map to render lines correctly
...
// ⚠️ MAKE SURE THIS IS IN YOUR .env FILE: VITE_MAPBOX_TOKEN=pk.eyJ...
...
// Route quality vs average cost helper
function getRouteQuality(cost: number, avgCost: number) {
  if (!avgCost || !isFinite(avgCost) || avgCost === 0) {
    return {
      label: "—",
...
export default function RoutesPage() {
  ...
  // ------------------------------------------
  // FIXED: ROUTE PROJECTION
  // ------------------------------------------
  const projectedRoutes = useMemo(() => {
    return coloredRoutes.map((r) => {
      // 1. Convert the raw grid steps (x,y) to Lat/Lng using our consistent function
      const convertedSteps = r.steps?.map((p) => convertPoint(p.x, p.y)) || [];

      // 2. FORCE alignment:
      // Instead of using r.full_path (which might be projected differently on backend),
      // we use our convertedSteps as the path source. This ensures lines touch dots.
      // We only fallback to full_path if steps are missing.
      const pathToUse =
        convertedSteps.length >= 2 ? convertedSteps : r.full_path || [];
  ...
  // Safe Access to Active Route
  const activeRoute = projectedRoutes[vehicleIndex] || null;
  const activeMetrics = routeMetrics[vehicleIndex] || null;
  ...
  // Full route line (Static gray line)
  ...
  const activeDistance = useMemo(() => {
    ...
    try {
      const total = turf.length(baseLine, { units: "kilometers" });
      const clamped = Math.max(0, Math.min(1, progress));
  ...
  // Partial progress line (Growing colored line)
  const partialLine = useMemo(() => {
    ...
    // Push all full segments processed so far
    ...
    if (start && end) {
      ...
      // Only push if different from last point to avoid 0-length segments at start
      const last = coords[coords.length - 1];
      if (!last || last[0] !== currLng || last[1] !== currLat) {
        coords.push([currLng, currLat]);
      }
    }

    // Turf requires at least 2 coordinates
  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      map.current.addSource("route-progress", {
        ...
        // @ts-expect-error not in type defs but required for line-gradient
        lineMetrics: true,
      });
  ...
  useEffect(() => {
    if (!playing || activeFullPath.length < 2 || !map.current) return;
  ...
  }, [playing, speed, activeFullPath.length]);

  // Move vehicle along route + camera follow
  // Uses activeFullPath to match the partialLine exactly
  useEffect(() => {
    if (!map.current || activeFullPath.length < 2) return;
  ...
  }, [progress, activeFullPath, activeRoute, cinematicMode]);
    ...
    return (
        <AdminLayout>
            <div className="p-10 text-center text-slate-400">
                <p>No routes found for this instance.</p>
            </div>
        </AdminLayout>
    )
  }
